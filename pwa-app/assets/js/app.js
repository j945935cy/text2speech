...
function composeWord(prefix, root, suffix) {
  ...
  if (suffix) {
    ...
    if (base.endsWith("y") && ["ness", "ful", "less", "ly"].includes(suffix)) {
      ...
      base.endsWith("e") &&
      startsWithVowel &&
      !["ee", "ye"].some((tail) => base.endsWith(tail))
    ) {
      base = base.slice(0, -1);
...
function renderMatches(items, activeWord = "") {
  ...
  if (!items.length) {
    els.matches.innerHTML =
      '<span class="muted">目前沒有完全符合的單字，試著只選字根，或移除一個條件。</span>';
    return;
  }
...
function renderConstructedWord() {
  ...
  if (!root) {
    ...
    els.constructedAnalysis.textContent = "目前還沒有字根，先輸入或選擇一個 root。";
...
function pickRandom() {
  const pool = entries.filter((entry) => entry.root && (entry.prefix || entry.suffix));
...
function bindEvents() {
  [els.prefixSelect, els.rootSelect, els.suffixSelect].forEach((control) => {
    control.addEventListener("change", renderConstructedWord);
    control.addEventListener("input", renderConstructedWord);
  });

  els.searchInput.addEventListener("input", renderWordList);

  els.speakCurrent.addEventListener("click", () => {
    const word = els.constructedWord.textContent.trim();
